...
export async function post(req, res) {
	...
	if (usernameRegex.test(username))
		return res.status(400).send({
			msg: "Invalid character in username." +
			"Only word characters, dashes and numbers allowed"
		});

	if (username.length > 20)
		return res.status(400).send({
			msg: "Too long username, max 20 characters"
		});

	if (typeof fullName === "string" && fullName.length > 40)
		return res.status(400).send({
			msg: "Too long full name, max 40 characters"
		});
	...
	if (typeof fullName === "string" && fullNameRegex.text(fullName))
		return res.status(400).send({
			...
			"Only word characters, spaces and dashes allowed"
		});
